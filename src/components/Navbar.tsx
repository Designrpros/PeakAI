    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-zinc-50/80 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm text-zinc-500">
          <Link href={`${BASE_PATH}/`} className="hover:text-zinc-950 transition-colors">hjem</Link>
          <Link href={`${BASE_PATH}/install`} className="hover:text-zinc-950 transition-colors">install</Link>
          <Link href={`${BASE_PATH}/documentation`} className="hover:text-zinc-950 transition-colors">documentation</Link>
        </nav>
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-40 bg-zinc-50 pt-20 px-6">
          <nav className="flex flex-col gap-6 text-2xl font-light">
            <Link href={`${BASE_PATH}/`} onClick={() => setOpen(false)}>hjem</Link>
            <Link href={`${BASE_PATH}/install`} onClick={() => setOpen(false)}>install</Link>
            <Link href={`${BASE_PATH}/documentation`} onClick={() => setOpen(false)}>documentation</Link>
          </nav>
        </div>
      )}
    </header>
